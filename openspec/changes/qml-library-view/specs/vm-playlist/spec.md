...
### Requirement: QML open helpers
`PlaylistViewModel` SHALL expose `Q_INVOKABLE` helper methods that accept a list index and emit the corresponding selection signal when the index is valid.

#### Scenario: Open local playlist by index
- **WHEN** QML calls `openLocalPlaylist(2)` and `localPlaylists[2]` exists
- **THEN** `localPlaylistSelected(id)` SHALL be emitted for that playlist

#### Scenario: Open NetEase playlist by index
- **WHEN** QML calls `openNeteasePlaylist(1)` and `neteasePlaylists[1]` exists
- **THEN** `neteasePlaylistSelected(summary)` SHALL be emitted for that playlist

#### Scenario: Invalid index ignored
- **WHEN** QML calls `openLocalPlaylist(999)` and the index is out of range
- **THEN** no selection signal SHALL be emitted
