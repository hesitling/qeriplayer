...
- [ ] 5.1 Create `src/qml/SettingsView.qml` with Page root, Flickable, Column layout
- [ ] 5.2 Implement General section with theme ComboBox (Light/Dark), audio quality ComboBox, download path TextField + Browse button
- [ ] 5.3 Implement NetEase Account section with login status label and Login/Logout button
- [ ] 5.4 Implement error label in Account section bound to `settingsVm.hasError` / `settingsVm.error.message`
- [ ] 5.5 Implement Storage section with "Clear Play History" button
- [ ] 5.6 Implement "About QeriPlayer" button
- [ ] 5.7 Add `FolderDialog` from `Qt.labs.platform` bound to download path Browse button

## 6. QML — LoginDialog

- [ ] 6.1 Create `src/qml/LoginDialog.qml` with Dialog root, TabBar (Password, SMS Code, QR Code)
- [ ] 6.2 Implement Password tab with phone/password TextFields and Login button calling `settingsVm.loginByPassword()`
- [ ] 6.3 Implement SMS Code tab with phone/captcha TextFields, Send Code button calling `settingsVm.sendCaptcha()`, Login button calling `settingsVm.loginByCaptcha()`
- [ ] 6.4 Bind Send Code button text to `settingsVm.canSendCaptcha` / `settingsVm.captchaCooldown` for countdown display
- [ ] 6.5 Implement QR Code tab with Image bound to `settingsVm.qrImageUrl`, status label bound to `settingsVm.qrLoginStatus`
- [ ] 6.6 Add 3-second Timer in QR tab calling `settingsVm.pollQrLogin()`, running when status is Waiting or Scanned
- [ ] 6.7 Call `settingsVm.generateQrLogin()` when QR tab becomes visible
- [ ] 6.8 Show "Refresh" button when status is Expired
- [ ] 6.9 Close dialog when `settingsVm.isNeteaseLoggedIn` becomes true
- [ ] 6.10 Call `settingsVm.cancelQrLogin()` and `settingsVm.clearError()` on dialog close

## 7. QML — AboutDialog

- [ ] 7.1 Create `src/qml/AboutDialog.qml` with Dialog root showing app name, `Qt.application.version`, Qt/QCoro/C++20 info, and OK button

## 8. Integration

- [ ] 8.1 Add `SettingsView.qml`, `LoginDialog.qml`, `AboutDialog.qml` entries to `src/qml/qml.qrc`
- [ ] 8.2 Replace placeholder `settingsPage` Component in `main.qml` with `SettingsView {}`
- [ ] 8.3 Add `Connections` block in `main.qml` for `settingsVm.errorChanged` to show toast
